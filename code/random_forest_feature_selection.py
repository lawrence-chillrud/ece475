# File: random_forest_feature_selection.py
# Author: Christian Duffee <christian.duffee@northwestern.edu>
# Date: 12/6/2022
...
# 6. Set up and run gridsearch of RandomForestClassifier hyperparameters if need be
if run_grid_search:
    # 6a. Initialize RandomForestClassifier classifier w/params that we've settled on already
    ...
    joblib.dump(gs.best_estimator_, 'data/generated/models/random_forest.pkl')
    results_df.to_csv('data/generated/models/random_forest.csv', index=False)

# 7. Load best model resulting from gridsearch
model = joblib.load('data/generated/models/random_forest.pkl')
...
for train_index, val_index in cv.split(X_train_val, y_train_val):
    ...
    for thresh in thresholds:
        ...
        # train model
        selection_model = RandomForestClassifier(**model.get_params())
