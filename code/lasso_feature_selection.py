# File: lasso_feature_selection.py
...
if run_grid_search:
    # 6a. Initialize XGBoost classifier w/params that we've settled on already
    ...
    joblib.dump(gs.best_estimator_, 'data/generated/models/lasso_final.pkl')
    results_df.to_csv('data/generated/models/lasso_results_df_final.csv', index=False)

# 7. Load best model resulting from gridsearch
model = joblib.load('data/generated/models/lasso_final.pkl')
...
thresholds = np.sort(model.coef_)
...
for train_index, val_index in cv.split(X_train_val, y_train_val):
    ...
    for thresh in thresholds:
        ...
        # train model
        selection_model = LogisticRegression(**model.get_params())
...
top_importances = model.coef_[0][best_feature_idx]
top_df = pd.DataFrame({'feature': top_features, 'importance': top_importances}).sort_values('importance', ascending=False)
top_df.to_csv('data/generated/feature_selection_output/lasso_features.csv', index=False)
